...
def list_(user: CurrentUser, db: DbSession):
    investments = service.list_investments(db, user.id)
...
def get(inv_id: str, user: CurrentUser, db: DbSession):
    inv = service.get_investment(db, user.id, inv_id)
...
def list_operations(inv_id: str, user: CurrentUser, db: DbSession):
    service.get_investment(db, user.id, inv_id)
    return [OperacaoOut.model_validate(o) for o in service.list_operations(db, user.id, inv_id)]


@router.post("/operations", response_model=OperacaoOut, status_code=201)
def create_operation(data: OperacaoIn, user: CurrentUser, db: DbSession):
    op = service.adicionar_operacao(db, user.id, data)
...
def refresh_quotes(user: CurrentUser, db: DbSession):
    investments = service.list_investments(db, user.id)
    tickers = [i.ticker for i in investments]
