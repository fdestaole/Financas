...
from app.modules.credit_cards.service import calcular_total_aberto, create_card
from app.modules.invoices.service import calcular_valor_total, list_invoices, pagar_fatura
...
def test_fluxo_completo_conta_cartao_transferencia(e2e_db):
    """Registrar -> conta -> cartão -> compras parceladas -> fatura -> transferência."""
    db = e2e_db
    ...
    assert calcular_saldo(db, acc) == Decimal("5000")
    ...
    assert calcular_saldo(db, acc) == Decimal("7500")

    # 5) Compra de R$ 1200 em 12x no cartão
    txs = criar_transacao(
        db,
        user.id,
        CompraCartaoIn(
    ...
    assert len(txs) == 12
    assert all(t.valor == Decimal("100.00") for t in txs)

    # 6) Verificar faturas
    invoices = list_invoices(db, user.id, card.id)
    assert len(invoices) == 12
    fatura_abr = next(
        i for i in invoices if i.mes_referencia == 4 and i.ano_referencia == 2026
    )
    assert calcular_valor_total(db, fatura_abr.id) == Decimal("100.00")

    # 7) Limite disponível
    aberto = calcular_total_aberto(db, card.id)
    assert aberto == Decimal("1200.00")
    assert card.limite - aberto == Decimal("1800")

    # 8) Pagar fatura abril
    pagar_fatura(db, user.id, fatura_abr.id, valor=Decimal("100"), data_pagamento=date(2026, 4, 20))
    assert calcular_saldo(db, acc) == Decimal("7400")
    ...
    criar_transacao(
        ...
        TransferenciaIn(
            ...
            valor=Decimal("200"),
            data=date(2026, 4, 25),
            bank_account_origem_id=acc.id,
            bank_account_destino_id=acc2.id,
        ),
    )
    assert calcular_saldo(db, acc) == Decimal("7200")
    assert calcular_saldo(db, acc2) == Decimal("200")
